...
module Fastlane
  module Actions
    ...
    class SetExpoAppfileAction < Action
      def self.run(params)
        # fastlane will take care of reading in the parameter and fetching the environment variable:
        UI.message "Path to app.json: #{params[:file_path]}/app.json"
        jsonString = JSON.pretty_generate Actions.lane_context[SharedValues::EXPO_APP_INFO]

        # sh "shellcommand ./path"

        File.write("#{params[:file_path]}/app.json", jsonString)
      end
